...
import {
  DomiciliarioService,
  Domiciliario,
} from '../../../service/domiciliario.service';

/**
 * Tabla CRUD para la gestión de domiciliarios desde el panel de administración.
 * Permite editar, eliminar, activar/desactivar y cambiar disponibilidad.
 */
@Component({
...
export class DomiciliarioCrudTableComponent implements OnInit {
  ...
  cargar(): void {
    this.domiciliarioService
      .findAll()
      .subscribe((list) => (this.domiciliarioList = list));
  }
  ...
  toggleActivo(d: Domiciliario): void {
    this.domiciliarioService
      .setActivo(d.id, !d.activo)
      .subscribe(() => this.cargar());
  }

  toggleDisponible(d: Domiciliario): void {
    this.domiciliarioService
      .setDisponibilidad(d.id, !d.disponible)
      .subscribe(() => this.cargar());
  }
}
